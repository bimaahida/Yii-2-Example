...
class Tanggal
{
    static function indonesian_date ($datetime, $format='d F Y') {
        ...
        $date = preg_replace ($pattern, $replace, $datetime->format($format));
        return $date;
    }
    static function add_month($datetime, $month){
        $month_later = clone $datetime;
        $month_later->add(new \DateInterval("P{$month}M"));
        return $month_later;
    }
    static  function yearDifference($start, $end){
    ...
    static function isDateFormat($s){
        return  Tanggal::isDateTimeformat($s) || Tanggal::isDateOnlyformat($s);
    }
    static function isDateTimeformat($s) {
    ...
    static function  isDateOnlyformat($s)
    {
        if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",$s))
        {
            return true;
        }else{
            return false;
